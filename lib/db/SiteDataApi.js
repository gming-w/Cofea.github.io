...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  let collectionQuery = pageRecordMap.collection_query
  ...
  // Fallback: 当 notion-client 的 getPage 未返回 collection_query 时，
  // 手动调用 queryCollection API 获取文章列表
  if (pageIds?.length === 0 && collectionId && viewIds?.length > 0) {
    console.warn('[Fallback] collection_query 为空，尝试 getCollectionData...')
    try {
      const queryResult = await notionAPI.getCollectionData(
        collectionId,
        viewIds[BLOG.NOTION_INDEX || 0],
        { limit: 999 }
      )
      const fallbackIds =
      ...
      if (fallbackIds.length > 0) {
        pageIds = fallbackIds
        // 合并 fallback 查询返回的 block 数据
        if (queryResult?.recordMap?.block) {
          block = Object.assign({}, block, queryResult.recordMap.block)
        }
        console.log('[Fallback] 成功获取', pageIds.length, '篇文章')
      }
    } catch (e) {
      console.error('[Fallback] getCollectionData 失败:', e.message)
    }
  }
